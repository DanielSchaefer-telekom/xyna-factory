...
**Note on role selection:** When multiple roles are returned from the JWT claims, the system now supports role prioritization via the `roleOrder` parameter. Roles are matched against the `roleOrder` list (case-sensitive) after prefix/suffix normalization. If no `roleOrder` is configured, the first normalized role from the token is selected.
...
3. If `roleOrder` is configured and non-empty:
   - Iterate through `roleOrder` list (in order) and match each role against the normalized roles using case-sensitive equality
...
**Important:** Role matching in `roleOrder` is case-sensitive. Roles from the token are normalized (prefix/suffix removed) before matching against the `roleOrder` list.
...
In the `/auth/externalUserLogin` flow, the token is read from the configured header (for example `OIDC_access_token`) and passed to `authorizeSession(...)`.
...
- `HEADER` assumes that a trusted reverse proxy (for example Apache with `mod_auth_openidc`) performs token validation and prevents header spoofing.
